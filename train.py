...
parser.add_argument('--data_dir', type=str, default='/mnt/fsl/dataset_rice1_all/')
...
#             writer.add_images('train_debug_img', frame_debug, iteration)
    ...
    if epoch % 1 == 0:
        print('we are testing on epoch in thin: ' + str(epoch))
        with torch.no_grad():
            psnr_list = []
            ssim_list = []
            recon_psnr_list = []
...
#                 imwrite(frame_out, output_dir +'/' +str(batch_idx) + '.png', range=(0, 1))
            ...
            if best_psnr < avr_psnr:
                best_psnr = avr_psnr
                best_epoch = epoch
                torch.save(MyEnsembleNet.state_dict(), os.path.join(args.model_save_dir,'rice1_epoch'+ str(epoch) + '.pkl'))
            print("best_psnr_rice1",best_psnr,"best_epoch_rice1",best_epoch)
